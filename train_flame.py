...
from model import FLAMEExpressionEncoder
...
def generate_inference_example(model, backbone, config, epoch, device):
    ...
    with torch.no_grad():
        ...
        hidden_states = backbone(pixel_values=image_tensor, return_dict=True).last_hidden_state
        predicted_expression = model(hidden_states).squeeze(0)
...
def train_model(config_path="config.yaml", restore_checkpoint=None):
    ...
    backbone = AutoModel.from_pretrained(config.model.model_name)
    ...
    for epoch in range(start_epoch, config.training.num_epochs):
        ...
        for batch_idx, (images, targets, blendshape_scores, shape_params, pose_params) in enumerate(train_loader):
            ...
            with autocast(dtype=torch.bfloat16):
                hidden_states = backbone(pixel_values=images, return_dict=True).last_hidden_state
